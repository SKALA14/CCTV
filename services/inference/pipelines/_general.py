...
WINDOW_SEC  = 10.0     # anomaly 트리거 후 VLM 호출까지 time window (초)


def _flush_camera(
    cam_id: str,
    buf: deque[tuple[str, str, str]],
    vlm: VLMClient,
) -> None:
    if len(buf) >= MIN_FRAMES:
        frame_paths  = [fp for _, fp, _ in buf][:BUFFER_SIZE]
        timestamp    = buf[0][2]
        prompt       = get_prompt(cam_id)
    ...
    for msg_id, frame_path, _ in buf:
