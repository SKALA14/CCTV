...
<template>
  <div class="p-6 max-w-5xl mx-auto">
    ...
    <!-- 구역별 요약 -->
    <h2 class="font-semibold text-sm mb-2" style="color: var(--text-primary);">구역별 요약</h2>
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
      <div v-for="z in overview" :key="z.zone"
        class="rounded-xl p-4" style="background: var(--bg-card); border: 1px solid var(--border);">
        <p class="font-medium text-sm mb-2" style="color: var(--text-primary);">{{ z.zone }}</p>
        <div class="text-xs space-y-1" style="color: var(--text-muted);">
          <div>카메라 <span class="tabular-nums">{{ z.camera_count }}</span>대</div>
          <button
            class="text-left hover:underline"
            style="color: var(--text-muted);"
            @click="openZoneEvents(z)"
          >오늘 이벤트 <span class="tabular-nums">{{ z.today_event_count }}</span>건 ›</button>
        </div>
      </div>
      <p v-if="!overview.length" class="text-sm col-span-full" style="color: var(--text-muted);">등록된 채널이 없습니다.</p>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr style="background: var(--bg-elevated);">
            <th class="text-left px-4 py-2.5 font-semibold" style="color: var(--text-muted);">구역</th>
        ...
        <tbody>
          <tr v-for="(d, i) in devices" :key="i" style="border-top: 1px solid var(--border);">
            <td class="px-4 py-2.5" style="color: var(--text-muted);">{{ d.zone }}</td>
          ...
          <tr v-if="!devices.length">
            <td colspan="5" class="px-4 py-6 text-center text-sm" style="color: var(--text-muted);">등록된 카메라가 없습니다.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- 계정 현황 -->
    <h2 class="font-semibold text-sm mb-2 mt-8" style="color: var(--text-primary);">계정 현황</h2>
    ...
    >
      <div class="rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col" style="background: var(--bg-card); border: 1px solid var(--border);">
        <div class="flex items-center justify-between px-5 py-3 flex-shrink-0" style="border-bottom: 1px solid var(--border);">
          <h3 class="font-bold text-sm" style="color: var(--text-primary);">{{ eventsModal.zone }} — 오늘 이벤트</h3>
          ...
          <table v-else class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="e in eventsModal.events" :key="e.event_id" style="border-top: 1px solid var(--border);">
                ...
                <td class="px-4 py-2" style="color: var(--text-primary);">{{ e.event_type }}</td>
