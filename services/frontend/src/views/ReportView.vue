    ...
    <template v-else-if="data">
      ...
      <div class="rounded-2xl p-5 mb-4" style="background: var(--bg-card); border: 1px solid var(--border);">
        ...
        <div class="flex items-end gap-1" style="height: 100px;">
          <div v-for="(d, i) in data.by_day" :key="i" class="flex-1 flex flex-col items-center justify-end gap-1">
            ...
            <span class="text-[8px]" style="color: var(--text-subtle);">{{ d.date.slice(3) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-2xl p-5 mb-4" style="background: var(--bg-card); border: 1px solid var(--border);">
        ...
        <div class="flex items-end gap-px" style="height: 70px;">
      ...
      <div class="rounded-2xl p-5" style="background: var(--bg-card); border: 1px solid var(--border);">
        <h2 class="text-sm font-semibold mb-3" style="color: var(--text-primary);">이벤트 多 카메라</h2>
...
const dangerOrder = ['critical', 'high', 'low', 'none']
const dangerLabel = { critical: '심각', high: '높음', low: '낮음', none: '정상' }
const dangerColor = { critical: '#b91c1c', high: '#dc2626', low: '#16a34a', none: '#6b7280' }
...
function barH(n, max) { return Math.round(((n || 0) / max) * (max === dayMax.value ? 90 : 60)) }
